...
cursor.execute("SELECT ID, TotalMarks FROM Users ORDER BY TotalMarks DESC")

rank = 1
prev_marks = None
rank_dict = {}

for index, (user_id, marks) in enumerate(cursor.fetchall(), start=1):
    if prev_marks is not None and marks < prev_marks:
        rank = index
    rank_dict[user_id] = rank
    prev_marks = marks


for user_id, rank in rank_dict.items():
